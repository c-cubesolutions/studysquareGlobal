...
$sb = iGuru_Theme_Helper::get_sidebar_params('single');
$single_type = iGuru_Theme_Helper::get_option('single_type_layout');
	...
	<div class="row<?php echo apply_filters( 'iguru_row_class', $row_class ); ?>">
		<div id='main-content' class="wgl_col-<?php echo apply_filters( 'iguru_column_class', $column ); ?>">
			...
			if ($nextPost || $previousPost) : ?>
				<div class="iguru-post-navigation"><?php
					if ( is_a($previousPost, 'WP_Post') ) {					
						...
						echo
							'<div class="prev-link_wrapper">',
								'<div class="info_wrapper">',
									'<a href="', esc_url(get_permalink($previousPost->ID)), '" title="', esc_attr($previousPost->post_title), '">',
										$img_prev_html,
										'<span class="prev-link-info_wrapper">',
											'<span class="prev_title">', wp_kses( $previousPost->post_title, $allowed_html ), '</span>',
											'<span class="meta-wrapper">',
												'<span class="date_post">', esc_html(get_the_time(get_option( 'date_format' ), $previousPost->ID)), '</span>',
											'</span>',
										'</span>',
									'</a>',
								'</div>',
					...
					if ( is_a($previousPost, 'WP_Post') || is_a($nextPost, 'WP_Post') ) :
						echo
							'<a class="back-nav_page" title="', esc_attr__( 'Back to previous page', 'iguru' ), '" onclick="location.href = document.referrer; return false;">',
								'<span></span><span></span>',
								'<span></span><span></span>',
							'</a>'
						;
			...
			$show_post_related = iGuru_Theme_Helper::get_option('single_related_posts');
				...
				// Related Posts
					...
					echo
						'<div class="iguru_module_title">',
							'<h4>',
								(!empty($mb_blog_title_r) ? esc_html($mb_blog_title_r) : esc_html__( 'Related Posts', 'iguru' )),
							'</h4>',
						'</div>'
					;
