...
if (!class_exists('iGuru_Theme_Helper')) {
    class iGuru_Theme_Helper{
        ...
        public static function get_option($name, $preset = null, $def_preset = null) {
            if (  class_exists( 'Redux' ) && class_exists( 'iGuru_Core_Public' ) ) {
                $preset = $preset == 'default' ? null : $preset;

                if ( ! $preset ) {

                    // Customizer
                    if (!empty($GLOBALS['iguru_set']) && $GLOBALS['iguru_set'] != NULL) {
                        $theme_options = $GLOBALS['iguru_set'];
                    } else {
                        $theme_options = get_option( 'iguru_set' );
                    }

                } else {
                    $theme_options = get_option( 'iguru_set_preset' );
                }
                
                if (empty($theme_options)) {
                    $theme_options = get_option( 'iguru_default_options' );
                }

                if ( ! $preset ) return isset($theme_options[$name]) ? $theme_options[$name] : null;  
                ...
                $default_option = get_option( 'iguru_default_options' );
        ...
        public static function bg_render($name,$check_key = false,$check_value = false) {
            $image = iGuru_Theme_Helper::get_option($name."_bg_image");
        ...
        public static function preloader(){
            if (self::get_option('preloader') == '1' || self::get_option('preloader') == true) {
                ...
                echo
                    '<div id="preloader-wrapper" ', esc_attr($bg_styles), '>',
		...
		public static function load_more($query = false, $name_load_more = '', $class = ''){
			$name_load_more = !empty($name_load_more) ? $name_load_more : esc_html__( 'Load More', 'iguru' );
        ...
        public static function header_preset_name(){
            ...
            if( !self::in_array_r($name_preset, get_option( 'iguru_set_preset' ))){                   
                $name_preset = 'default';
            }
		...
		public static function get_sidebar_params($cpt = 'page') {
			$cpt = apply_filters( 'iguru/theme-helper/get-sidebar-params/cpt', $cpt);

			$layout = self::get_option( $cpt.'_sidebar_layout');
			$sidebar = self::get_option( $cpt.'_sidebar_def');
			$sidebar_width = self::get_option($cpt.'_sidebar_def_width');
			$sticky_sidebar = self::get_option($cpt.'_sidebar_sticky');
			$sidebar_gap = self::get_option($cpt.'_sidebar_gap');

			$output = array();
			$sidebar_class = $sidebar_style = '';

            if ( !class_exists('iGuru_Core') ) {
			...
			if ( is_active_sidebar( $sidebar ) && ($layout == 'left' || $layout == 'right') ) {
				$output['layout'] = $layout;
				$output['id'] = $sidebar;
				$output['row_class'] = $layout != 'none' ? ' sidebar_'.esc_attr($layout) : '';
				$output['column'] = $column;
				$output['class'] = $sidebar_class;
				$output['style'] = $sidebar_style;
						
				return $output;
			}
		}

		public static function render_sidebar($sidebar) {
			echo '<div class="sidebar-container', $sidebar['class'], ' wgl_col-', (12 - (int)$sidebar['column']), '" ', $sidebar['style'], '>';
				echo '<aside class="sidebar">';
					do_action( 'iguru/theme-helper/render-sidebar/before-widgets' );
        ...
        public static function posted_meta_on() {
            global $post;
            $text_string = '<span><time class="entry-date published" datetime="%1$s">%2$s</time></span><span>' . esc_html__('Published in', 'iguru') . ' <a href="%3$s" rel="gallery">%4$s</a></span>';
            ...
            if ( $metadata ) {
                printf( '<span class="full-size-link"><span class="screen-reader-text">%1$s </span><a href="%2$s" title="%2$s">%1$s %3$s &times; %4$s</a></span>',
                    esc_html_x( 'Full size', 'Used before full size attachment link.', 'iguru' ),
            ...
            edit_post_link(
                sprintf(
                    ...
                    wp_kses( __( 'Edit<span class="screen-reader-text"> "%s"</span>', 'iguru' ), $allowed_html ) ,
                        get_the_title()
        ...
        public function vc_iconpicker_type_flaticon( $icons ) {
            $flaticon_icons = [
                ...
                array('flaticon-paper-plane-4' => 'paper-plane-4'),
                ...
                array('flaticon-highlighter' => 'highlighter'),
                array('flaticon-dna-1' => 'dna-1'),
                array('flaticon-chalkboard' => 'chalkboard'),
                array('flaticon-trophy' => 'trophy'),
                array('flaticon-sharpener' => 'sharpener'),
                array('flaticon-american-football' => 'american-football'),
                array('flaticon-shoe' => 'shoe'),
                array('flaticon-backpack' => 'backpack'),
                array('flaticon-quotation' => 'quotation'),
                array('flaticon-folder' => 'folder'),
                array('flaticon-microscope' => 'microscope'),
                array('flaticon-cut' => 'cut'),
                array('flaticon-magnet' => 'magnet'),
                array('flaticon-sharpener-1' => 'sharpener-1'),
                array('flaticon-domain-registration' => 'domain-registration'),
                array('flaticon-profile' => 'profile'),
                array('flaticon-search-8' => 'search-8'),
                array('flaticon-night' => 'night'),
                array('flaticon-compass-1' => 'compass-1'),
                array('flaticon-protractor' => 'protractor'),
                array('flaticon-blockchain' => 'blockchain'),
                array('flaticon-research' => 'research'),
                array('flaticon-brain-1' => 'brain-1'),
                array('flaticon-shield' => 'shield'),
                array('flaticon-blockchain-1' => 'blockchain-1'),
                ...
                array('flaticon-notebook-and-pencil' => 'notebook-and-pencil'),
                array('flaticon-paper-plane' => 'paper-plane'),
                array('flaticon-flask' => 'flask'),
                array('flaticon-paper-and-pencil' => 'paper-and-pencil'),
                array('flaticon-abc' => 'abc'),
                array('flaticon-pencil-and-paintbrush' => 'pencil-and-paintbrush'),
