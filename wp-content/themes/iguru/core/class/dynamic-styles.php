...
/**
*  iGuru Dynamic Styles
*
*
*  @class        iGuru_dynamic_styles
*  @version      1.0
*  @category     Class
*  @author       WebGeniusLab
*/
...
class iGuru_dynamic_styles{
	...
	public function register_script(){
		...
		$this->use_minify = iGuru_Theme_Helper::get_option('use_minify') ? '.min' : '';
		// Register action
	...
	public function css_reg(){
	    /* Register CSS */
	    wp_enqueue_style('iguru-default-style', get_bloginfo('stylesheet_url'));
	    // Flaticon register
		...
		wp_enqueue_style('iguru-main', $this->gtdu . '/css/main'.$this->use_minify.'.css');
		wp_enqueue_style('swipebox', get_template_directory_uri() . '/js/swipebox/css/swipebox.min.css');
	}
	/* Register JS */
	public function js_reg(){

		wp_enqueue_script('iguru-theme-addons', $this->gtdu . '/js/theme-addons'.$this->use_minify.'.js', array('jquery'), false, true);
		wp_enqueue_script('iguru-theme', $this->gtdu . '/js/theme.js', array('jquery'), false, true);

	    wp_localize_script( 'iguru-theme', 'wgl_core', array(
	        ...
	        'like' => esc_html__( 'Like', 'iguru' ),
	        'unlike' => esc_html__( 'Unlike', 'iguru' )
	        ) );
	...
	public function admin_css_reg(){
		...
		wp_enqueue_style('font-awesome', $this->gtdu . '/css/font-awesome.min.css');
		// Main admin styles
		wp_enqueue_style('iguru-admin', $this->gtdu . '/core/admin/css/admin.css');
		...
		wp_enqueue_style('iguru-vc-backend-style', $this->gtdu . '/core/admin/css/wgl-vc-backend.css');
	}
	...
	public function add_style() {
		...
		/* Body Style
		/*-----------------------------------------------------------------------------------*/
		$page_colors_switch = iGuru_Theme_Helper::options_compare('page_colors_switch','mb_page_colors_switch','custom');
		if ($page_colors_switch == 'custom') {
			$theme_color = iGuru_Theme_Helper::options_compare('page_theme_color','mb_page_colors_switch','custom');
			$theme_secondary_color = iGuru_Theme_Helper::options_compare('page_theme_secondary_color','mb_page_colors_switch','custom');
			$bg_body = iGuru_Theme_Helper::options_compare('body_background_color','mb_page_colors_switch','custom');
			// Go top color
			$scroll_up_bg_color = iGuru_Theme_Helper::options_compare('scroll_up_bg_color','mb_page_colors_switch','custom');
			$scroll_up_arrow_color = iGuru_Theme_Helper::options_compare('scroll_up_arrow_color','mb_page_colors_switch','custom');
		} else {
			...
			$bg_body = esc_attr(iGuru_Theme_Helper::get_option('body-background-color'));
			// Go top color
			$scroll_up_bg_color = iGuru_Theme_Helper::get_option('scroll_up_bg_color');
			$scroll_up_arrow_color = iGuru_Theme_Helper::get_option('scroll_up_arrow_color');
		}
		$bg_body = !empty($bg_body) ? 'background: '.esc_attr($bg_body).';' : '';
		...
		// Add Heading h[1-6] variables
		for ($i = 1; $i <= 6; $i++) {
		    ${'header-h'.$i} = iGuru_Theme_Helper::get_option('header-h'.$i);
		...
		/*-----------------------------------------------------------------------------------*/
		$menu_font = iGuru_Theme_Helper::get_option('menu-font');
		...
		$sub_menu_font = iGuru_Theme_Helper::get_option('sub-menu-font');
		...
		$secondary_font = iGuru_Theme_Helper::get_option('secondary-font');
		if (!empty($secondary_font)) {
			$secondary_font_family = esc_attr($secondary_font['font-family']);
			$secondary_font_weight = esc_attr((int)$secondary_font['font-weight']);
		} else {
			$secondary_font_family = $secondary_font_weight = '';
		}
		...
		$name_preset = iGuru_Theme_Helper::header_preset_name();
		...
		$menu_color_top = iGuru_Theme_Helper::get_option('header_top_color', $name_preset, $def_preset);
		if (!empty($menu_color_top['rgba'])) {
	        $menu_color_top = !empty($menu_color_top['rgba']) ? esc_attr($menu_color_top['rgba']) : '';
	    }

		$menu_color_middle = iGuru_Theme_Helper::get_option('header_middle_color', $name_preset, $def_preset);
		if (!empty($menu_color_middle['rgba'])) {
			$menu_color_middle = !empty($menu_color_middle['rgba']) ? esc_attr($menu_color_middle['rgba']) : '';
		}

		$menu_color_bottom = iGuru_Theme_Helper::get_option('header_bottom_color', $name_preset, $def_preset);
	    ...
	    $sub_menu_color = iGuru_Theme_Helper::get_option('sub_menu_color' ,$name_preset, $def_preset);
	    $sub_menu_bg = iGuru_Theme_Helper::get_option('sub_menu_background' ,$name_preset, $def_preset);
	    $sub_menu_bg = $sub_menu_bg['rgba'];


		$mobile_sub_menu_bg = iGuru_Theme_Helper::get_option('mobile_sub_menu_background');
		$mobile_sub_menu_bg = $mobile_sub_menu_bg['rgba'];
		$mobile_sub_menu_color = iGuru_Theme_Helper::get_option('mobile_sub_menu_color');

		$hex_header_font_color = iGuru_Theme_Helper::HexToRGB($header_font_color);
		$hex_theme_color = iGuru_Theme_Helper::HexToRGB($theme_color);
		$hex_theme_color_secondary = iGuru_Theme_Helper::HexToRGB($theme_secondary_color);
		$hex_theme_content =  iGuru_Theme_Helper::HexToRGB($content_color);
	    ...
	    $footer_text_color = iGuru_Theme_Helper::get_option('footer_text_color');
	    $footer_heading_color = iGuru_Theme_Helper::get_option('footer_heading_color');

	    $copyright_text_color = iGuru_Theme_Helper::options_compare('copyright_text_color','mb_copyright_switch','on');

		// Page Title Background Color
		$page_title_bg_color = iGuru_Theme_Helper::get_option('page_title_bg_color');
		$hex_page_title_bg_color = iGuru_Theme_Helper::HexToRGB($page_title_bg_color);
		...
		if ( is_single() ) {
			$layout_single = iGuru_Theme_Helper::options_compare('shop_single_image_layout', 'mb_product_layout', 'custom');
			if($layout_single === 'with_background'){
				$layout_bg = iGuru_Theme_Helper::get_option('shop_layout_with_background');
		...
		if ( class_exists('WooCommerce') ) array_push($files, 'shop');
		...
		/*-----------------------------------------------------------------------------------*/    
		$mobile_header = iGuru_Theme_Helper::get_option('mobile_header');
		...
		// Set Queries width to apply mobile style
		$header_queries = iGuru_Theme_Helper::get_option('header_mobile_queris', $name_preset, $def_preset);
		$mobile_over_content = iGuru_Theme_Helper::get_option('mobile_over_content');
		...
		/*-----------------------------------------------------------------------------------*/   
		$page_title_resp = iGuru_Theme_Helper::get_option('page_title_resp_switch');
		$mb_cond_logic = false; 
		...
		if ($page_title_resp == '1') {
			...
			$page_title_height = iGuru_Theme_Helper::get_option('page_title_resp_height');			
			$page_title_height = $page_title_height['height'];

			$page_title_queries = iGuru_Theme_Helper::options_compare('page_title_resp_resolution', 'mb_page_title_resp_switch', $mb_cond_logic);
			
			$page_title_padding = iGuru_Theme_Helper::options_compare('page_title_resp_padding', 'mb_page_title_resp_switch', $mb_cond_logic);
			
			if($mb_cond_logic == '1'){
				$page_title_height = rwmb_meta('mb_page_title_resp_height');
			}

			$page_title_font = iGuru_Theme_Helper::options_compare('page_title_resp_font', 'mb_page_title_resp_switch', $mb_cond_logic);
			$page_title_breadcrumbs_font = iGuru_Theme_Helper::options_compare('page_title_resp_breadcrumbs_font', 'mb_page_title_resp_switch', $mb_cond_logic);
			$page_title_breadcrumbs_switch = iGuru_Theme_Helper::options_compare('page_title_resp_breadcrumbs_switch', 'mb_page_title_resp_switch', $mb_cond_logic);
		...
		/* Footer page CSS
		/*-----------------------------------------------------------------------------------*/
		$footer_switch = iGuru_Theme_Helper::get_option('footer_switch');
		if ($footer_switch) {
			$footer_content_type = iGuru_Theme_Helper::get_option('footer_content_type');
			if (class_exists( 'RWMB_Loader' ) && get_queried_object_id() !== 0) {
			...
			if ( $footer_content_type == 'pages' ) {
				$footer_page_id = iGuru_Theme_Helper::options_compare('footer_page_select');
				if ( $footer_page_id ) {
		...
		wp_add_inline_style( 'iguru-main', $css );
...
if (!function_exists('iguru_dynamic_styles')) {
    function iguru_dynamic_styles() {
        return iGuru_dynamic_styles::instance();
    }
}

iguru_dynamic_styles()->register_script();
iguru_dynamic_styles()->init_style();
