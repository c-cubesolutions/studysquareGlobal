...
if ( ! class_exists('iGuru_Single_Post') ) {

	class iGuru_SinglePost {
		/**
		 * @var SinglePost
		 */
		...
		public function set_data_image ($link_feature = false, $image_size = 'full', $aq_image = false ) {
			...
			if (empty($media)) $this->meta_info_render = false;
		...
		public function render_featured ( $link_feature = false, $image_size = 'full', $aq_image = false, $meta_args = false, $meta_to_show = false ) {
			$output = $class_media_part = $oembed_error = '';

			if (class_exists( 'RWMB_Loader' )) {
				switch($this->post_format) {
					case 'gallery' : $media = $this->featured_gallery($link_feature, $image_size); break;
					case 'video'   : $media = $this->featured_video($image_size, $aq_image); break;
					...
					default        : $media = $this->featured_image($link_feature, $image_size, $aq_image); break;
				}
			} else {
				$media = $this->featured_image($link_feature, $image_size, $aq_image);
			}
			...
			if ( ! empty($media) && ! $oembed_error ) :
				echo '<div class="blog-post_media">';
					echo '<div class="blog-post_media_part', esc_attr($class_media_part), '">', $media, '</div>';
					if ( $meta_args ) {
		...
		public function videobox() {
			if ( $this->post_format === 'video' ) {
				...
				if ($video_style != 'bg_video') {
					...
					$uniqrel = uniqid();
					echo '<div class="iguru_module_videobox button_align-center">';
		...
		public function render_bg ( $link_feature = false, $image_size = 'full',$aq_image = false, $data_animation = null ) {
			...
			if ($this->post_format === 'gallery' && !is_single()) {
				...
				if ($this->post_format === 'video' && $video_style == 'bg_video') {
					echo iGuru_Theme_Helper::render_html($media);
				} else {
			...
			if (is_single() && $video_style != 'bg_video') {
				...
				if ($video_style == 'bg_video') {
					...
					if (has_post_thumbnail()) {
						...
						$output .= '<div class="iguru_module_videobox button_align-center animation_circles with_image always-run-animation">';
		...
		public function featured_gallery ($link_feature, $image_size) {
			...
			if ( empty($gallery_data) ) return; // get out of the function in case of empty data
			
			$arr = $this->image_size_render($image_size);  
			extract($arr);
			$class = 'blog-post_media-slider_slick iguru_carousel_slick fade_slick';
			$class_module = ' iguru_module_carousel';
			ob_start();
			?>
		...
		public function featured_quote () {
			...
			?>
				<div class="blog-post_quote-author">
					<?php
					echo iGuru_Theme_Helper::render_html($autor_avatar);
		...
		public function image_size_render($image_size) {
			$arr = [];
			switch ($image_size) { // ratio = 1.2333
				case 'iguru-420w':  $arr['width'] = '420';  $arr['height'] = '300'; break;
				case 'iguru-440w':  $arr['width'] = '440';  $arr['height'] = '356'; break;
				case 'iguru-600w':  $arr['width'] = '600';  $arr['height'] = '486'; break; 
				default:
				case 'iguru-1170w': $arr['width'] = '1170'; $arr['height'] = '728'; break;
			}
			return $arr;
		}
		...
		public function image_size_render_bg($image_size) {
			$arr = [];
			switch ($image_size) {
				case 'iguru-740-560': $arr['width'] = '740';  $arr['height'] = '560'; break;
				case 'iguru-700-700': $arr['width'] = '700';  $arr['height'] = '700'; break;
				default: $arr['width'] = '1170'; $arr['height'] = '728'; break;
			}
			return $arr; 
		}
		...
		public function render_post_meta ($args = false, $shortened = false) {
			$this->set_post_meta($args);
			if ( $this->show_author_meta || $this->show_category_meta ) :
				echo '<div class="meta-wrapper post-cats-author">';
			...
			if ( $this->show_date_meta ) :
				echo '<div class="meta-wrapper post-date">';
					if ($this->show_date_meta)
						echo
			...
			if ( $this->show_comments_meta ) :
				...
				echo
					'<div class="meta-wrapper post-comments">',
						'<span class="comments_post">',
							'<a href="', esc_url(get_comments_link()), '" title="', esc_attr__( 'Leave a reply', 'iguru' ), '">',
								esc_html($comments_num),
								' ',
								$shortened ? '' : esc_html( _n('Comment', 'Comments', $comments_num, 'iguru')  ),
							'</a>',
						'</span>',
					'</div>'
				;
		...
		public function render_post_cats () {
			...
			if ( $categories = get_the_category() ) :
				foreach ($categories as $category) {
					...
					echo
						'<span', (!empty($color) ? ' style="color:#'.esc_attr($color).';"' : "" ), '>',
							'<a href="', esc_url(get_category_link($category->term_id)),'"',(!empty($color) ? ' style="color: #'.esc_attr($color).';"' : "" ),'>',
								$category->cat_name,
							'</a>',
						'</span>'
					;
		...
		public function render_excerpt ($symbol_count = false, $shortcode = false, $read_more = false, $read_more_text = false) {
			...
			if ((bool)iGuru_Theme_Helper::get_option('blog_post_listing_content') || $shortcode) {
				$post_content = preg_replace( '~\[[^\]]+\]~', '', $post_content);
				$post_content_stripe_tags = strip_tags($post_content);
				$output = iGuru_Theme_Helper::modifier_character($post_content_stripe_tags, $symbol_count, "...");
			} else {
				$output = $post_content;
			}
		...
		public function get_post_views($postID) {
			...
			echo '<div class="wgl-views" title="Views amount">';
			if ($count == '') :
				delete_post_meta($postID, $count_key);
				add_post_meta($postID, $count_key, '0');
				echo '<span class="counts">', esc_html(0), '</span>',
					 '<span class="counts_text">', esc_html__( ' View', 'iguru' ), '</span>';
			else :
				echo '<span class="counts">', esc_html($count), '</span>',
				_nx(
					'<span class="counts_text"> View</span>',
					'<span class="counts_text"> Views</span>',
					$count,
					'views counter',
					'iguru'
				);
		...
		public function render_author_info() {
			...
			$name_html = !empty($user_first) || !empty($user_last) ? '<h5 class="author-info_name">'.'<span class="author-excerpt_name">'.esc_html__( 'About', 'iguru' ).'</span>'.$user_first.' '.$user_last.'</h5>' : '';
