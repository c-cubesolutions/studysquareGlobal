...
if ( $custom_pag_color || $custom_prev_next_color || !empty($pag_offset) || !empty($prev_next_offset)) {
	$carousel_id = uniqid( "iguru_carousel_" );
}
...
ob_start();
	if ( $custom_pag_color ) {
		echo
			"#$carousel_id.pagination_circle .slick-dots li button,
	...
	if ( $custom_prev_next_color ) {
		...
		$bg_color_idle_shadow = '0px 0px 0px 0px rgba('.(!empty($prev_next_bg_idle) ? '0,0,0,0' : '0,0,0,0').')';
		$bg_color_hover_shadow = '0px 0px 0px 0px rgba('.(!empty($prev_next_bg_hover) ? '0,0,0,0' : '0,0,0,0').')';
		echo
	...
	if ( !empty($pag_offset) ) {
		echo "#$carousel_id.iguru_module_carousel .slick-dots{
				  margin-top: ", esc_attr((int)$pag_offset), "px;
			  }";
	}
	if ( $custom_prev_next_offset ) {
		if ( !empty($prev_next_offset) ) {
			echo "#$carousel_id.iguru_module_carousel .slick-next,
				  #$carousel_id.iguru_module_carousel .slick-prev{
...
// Render
$output = '<div class="iguru_module_carousel-wrapper">';
	$output .= '<div'.$carousel_id_attr.' class="iguru_module_carousel'.esc_attr($carousel_wrap_classes).'">';
		$output .= '<div class="iguru_carousel_slick'.$carousel_classes.'"'.$data_attribute.'>';	
