...
$icon_output = $counter_icon = $counter_value = '';
...
switch (true) {
	case !empty($counter_offsets_css):
	case !empty($icon_offsets_css):
	case (bool)$custom_title_color:
	case (bool)$custom_value_color:
	case (bool)$custom_icon_color:
	case !empty($icon_bg_color_type):
	case (bool)$custom_icon_border_color:
	case (bool)$add_value_text_stroke:
	case (bool)$custom_divider_color:
		$counter_id = uniqid( "iguru_counter_" );
		$counter_id_attr = ' id='.$counter_id;
		break;
	default:
		$counter_id = $counter_id_attr = '';
		break;
}
// Custom styles
ob_start();
	...
	if ( !empty($icon_bg_color_type) ) {
		switch ( $icon_bg_color_type ) {
			...
			case 'gradient':
				...
				echo
					"#$counter_id .counter_icon_container {",
						$gradient_idle,
					'}',
					"#$counter_id .counter_icon_container:after {
						content: '';
	...
	if ( $custom_divider_color )
		echo "#$counter_id:after {
				  background-color: ", (!empty($divider_color) ? esc_attr($divider_color) : 'transparent'), ";
			  }";
	...
	if ( $add_shadow ) {
		...
		switch ( $shadow_appearance ) {
			case 'before_hover':
				echo
					"#$counter_id .counter_content_wrapper { ", $counter_shadow, " }",
					"#$counter_id:hover .counter_content_wrapper { box-shadow: none }"
				;
				break;
			case 'on_hover':
				echo
					"#$counter_id .counter_content_wrapper { box-shadow: none }",
					"#$counter_id:hover .counter_content_wrapper { ",
						$counter_shadow,
...
$counter_wrap_classes .= $add_counter_divider ? ' counter_divider' : '';
$counter_wrap_classes .= $animation_class;
$counter_wrap_classes .= !empty($extra_class) ? ' '.$extra_class : '';

// Render Google Fonts
extract( iGuru_GoogleFontsRender::getAttributes( $atts, $this, [ 'google_fonts_title', 'google_fonts_count_value' ] ) );
...
$count_value_font_size = !empty($value_size) ? 'font-size:'.esc_attr((int)$value_size).'px; ' : '';
...
// Value styles output
$count_value_styles = $count_value_font_size . $count_value_font . $value_font_weight;
...
// Render html
$output = '<div'.esc_attr($counter_id_attr).' class="iguru_module_counter'.esc_attr($counter_wrap_classes).'">';
