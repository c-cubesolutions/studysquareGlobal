...
$videobox_id = uniqid( "iguru_video_" );
...
switch ($animation_style) {
	...
	case 'animation_ring_rotate':
		$svg_ring_circle_color = !empty($title_color) ?  'rgba('.iGuru_Theme_Helper::HexToRGB($title_color).', 0.1)' : 'rgba(0,0,0,0.1)';
...
// Triangle styles
$triangle_size_style = !empty($triangle_size) ? ' width="'.esc_attr((int)$triangle_size).'px" height="'.esc_attr((int)$triangle_size).'px" ' : ' width="34%" height="34%"';
$triangle_color_style = ' fill="'.(!empty($triangle_color) ? esc_attr($triangle_color) : 'white').'"';

// Triangle svg output
switch ($triangle_shape = 'sharp') {
	case 'sharp':
...
$output = '<div'.$videobox_id_attr.' class="iguru_module_videobox'.esc_attr($video_wrap_classes).'">';
