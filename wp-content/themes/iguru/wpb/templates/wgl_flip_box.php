<?php
	...
	$defaults = array(
		...
		'back_descr_color' => '',
		'add_read_more' => false,
		'read_more_color_idle' => $theme_color,
	...
	if ( $front_offsets_css || $back_offsets_css || $add_read_more ) {
		$flipbox_id = uniqid( "iguru_flipbox_" );
	...
	ob_start();
		...
		if ( $add_read_more ) {
			if ( $read_more_color_idle )
				echo "#$flipbox_id .flipbox_button { color: ", esc_attr($read_more_color_idle), "; }";

			if ( $read_more_color_hover )
				echo "#$flipbox_id .flipbox_button:hover { color: ", esc_attr($read_more_color_hover), "; }";
		}
	$styles = ob_get_clean();
	...
	$flipbox_button = (bool)$add_read_more ? '<a class="flipbox_button" '.$button_attr.'>'.esc_html($read_more_text).'</a>' : '';
	...
	// Render html
	$output = '<div'.$flipbox_id_attr.' class="iguru_module_flipbox'.esc_attr($flipbox_wrap_classes).'">';
