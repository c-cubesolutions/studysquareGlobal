...
foreach ( $values as $data ) :
	...
	$new_data['bar_color_type'] = isset( $data['bar_color_type'] ) ? $data['bar_color_type'] : '';
	$new_data['bar_color'] = isset( $data['bar_color'] ) ? esc_attr($data['bar_color']) : '';
	$new_data['bar_gradient_start'] = isset( $data['bar_gradient_start'] ) ? esc_attr($data['bar_gradient_start']) : '';
	$new_data['bar_gradient_end'] = isset( $data['bar_gradient_end'] ) ? esc_attr($data['bar_gradient_end']) : '';
	$new_data['bar_bg_color'] = isset( $data['bar_bg_color'] ) ? esc_attr($data['bar_bg_color']) : '';
	$new_data['label_color'] = isset( $data['label_color'] ) ? esc_attr($data['label_color']) : '';
	$new_data['value_color'] = isset( $data['value_color'] ) ? esc_attr($data['value_color']) : '';
...
foreach ( $item_data as $item_d ) :
	...
	ob_start();
		...
		if ( $item_d['bar_color_type'] == 'gradient' ) {
			...
			$gradient .= 'background: -webkit-radial-gradient(100% 150%, circle farthest-corner, '.$item_d['bar_gradient_start'].' 10%, '.$item_d['bar_gradient_end'].' 50%);';
			$gradient .= 'background: radial-gradient(circle farthest-corner at 100% 150%, '.$item_d['bar_gradient_start'].' 10%, '.$item_d['bar_gradient_end'].' 50%)';
			echo "#$progress_id .progress_bar {
					  $gradient;
				  }";
		}
	...
	$content .= '<div'.$progress_attr.' class="progress_wrap">';
		...
		$content .= '<div class="progress_bar_wrap">';
		    $content .= '<div class="progress_bar' . $progress_bar_point . '" data-width="'.$item_d['point_value'].'"></div>';
		$content .= '</div>';
    $content .= '</div>';
endforeach;

$output = '<div class="iguru_module_progress_bar'.esc_attr($progress_classes).'">';
    $output .= $content;
$output .= '</div>';
