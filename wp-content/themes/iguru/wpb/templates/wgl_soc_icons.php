<?php
    ...
    ob_start();
        if ((bool)$add_bg) {
            $border = !empty($border) ? 'border: '.esc_attr((int)$border).'px solid '.(!empty($all_border_color) ? esc_attr($all_border_color) : 'transparent').';' : '';
        ...
        if (!empty($icons_location) &&  $icons_location == 'display_inline') {
    ...
    $social_wrap_classes .= !empty($icons_location) && $icons_location == 'full_width' ? ' full_width' : '';
	$social_wrap_classes .= !empty($animation_class) ? ' '.$animation_class : '';
	$social_wrap_classes .= !empty($extra_class) ? ' '.$extra_class : '';

	$social_wrap_styles = (!empty($icon_gap) && !empty($icons_location) &&  $icons_location == 'display_inline') ? 'margin-left:-'.($icon_gap/2).'px; margin-right:-'.($icon_gap/2).'px;' : '';
    if (!empty($icons_location) &&  $icons_location == 'display_inline') {
    ...
    foreach ( $item_data as $item_d ) {
        ...
        $title_text = !empty( $item_d['title'] ) ? esc_attr($item_d['title']) : "";
        ...
        ob_start();
            if ((bool)$item_d['custom_colors']) {
                echo ".iguru_module_social #$social_id{
                          color: ".$item_d['icon_color'].";
                      }";
                echo ".iguru_module_social #$social_id:hover{
            ...
            if (!empty($icons_location) && $icons_location == 'full_width') {
                $icon_selector = str_replace(' ', '.', $item_d['icon']);
                echo '#'.$soc_icon_wrap_id.' .soc_icon.'. $icon_selector .':before {font-size: 0;}';
                echo '#'.$soc_icon_wrap_id.' .soc_icon.'.$icon_selector .':hover:before{display: inline; font-size: 1.2em;}';
            }
        $styles = ob_get_clean();
        iGuru_shortcode_css()->enqueue_iguru_css($styles);
        ...
        if (!empty($icons_location) && $icons_location == 'full_width') {
                $content .= '<a'.$social_attr.' href="'.esc_url($item_d['link']).'" class="soc_icon_fullwidth soc_icon '.$item_d['icon'].'" '.$icon_style.' '.$new_tab.' '.$title_tag.'><span class="text">'.$title_text.'</span></a>';
        }
        else {
    ...
    $output = '<div '.esc_attr($soc_icon_wrap_id_attr).' class="iguru_module_social'.esc_attr($social_wrap_classes).'" '.$social_wrap_styles.'>';
        $output .= $content;
    $output .= '</div>';
