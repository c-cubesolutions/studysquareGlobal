...
$defaults = array(
	...
	// Styles
	'heading_tag' => 'h3',
	'heading_margin_bottom' => '13px',
	'custom_fonts_blog_content' => '',
	'google_fonts_blog' => '',
	'custom_fonts_blog_headings' => '', 
	'google_fonts_blog_headings' => '',
	'custom_main_color' => '#abaebe',
	'custom_read_more_color' => $theme_color,
	'custom_hover_read_more_color' => $main_font_color,
	'custom_headings_color' => $header_font_color,
	'custom_hover_headings_color' => $theme_color,
	'custom_content_color' => $main_font_color,
...
switch (true) {
	...
	case (bool)$custom_fonts_blog_size_headings: $custom_style = true; break;
	default: $custom_style = '';
}

// Module unique id
$blog_id = (bool)$custom_style ? uniqid( 'blog_module_' ) : '';

// Render Google Fonts
if ( (bool)$custom_fonts_blog_content || (bool)$custom_fonts_blog_headings ) {

	$blog_value_font = $blog_value_font_headings = '';
	extract( iGuru_GoogleFontsRender::getAttributes( $atts, $this, array('google_fonts_blog', 'google_fonts_blog_headings')));
	...
	if ( !empty($styles_google_fonts_blog_headings) ) {
		...
		$styles .= "#$blog_id.blog-posts .blog-post_title {
...
if ( (bool)$custom_style ) {
	...
	$heading_font_size_css = !empty($heading_font_size) ? ' font-size:'.esc_attr((int)$heading_font_size).'px;' : '';
	$heading_line_height = !empty($heading_font_size) ? ' line-height:'.esc_attr((int)$heading_line_height).'px;' : '';
	$heading_font_weight = !empty($heading_font_size) ? ' font-weight:'.esc_attr((int)$heading_font_weight).';' : '';
	
	$content_font_size_css = !empty($content_font_size) ? ' font-size:'.esc_attr((int)$content_font_size).'px;' : '';
	$content_line_height = !empty($content_line_height) ? ' line-height:'.esc_attr((int)$content_line_height).'px;' : '';
	$background_color = !empty($custom_image_mask_color) ? ' background-color:'.esc_attr($custom_image_mask_color).';' : '';
	$background_color_items = !empty($custom_bg_color) ? ' background-color:'.esc_attr($custom_bg_color).';' : '';
	$background_color_hover = !empty($custom_image_hover_mask_color) ? ' background-color:'.esc_attr($custom_image_hover_mask_color).';' : '';
	...
	if ( (bool)$custom_fonts_blog_size_headings && (bool)$heading_font_size_css ) {
		echo "#$blog_id.blog-posts .blog-post_title,
			  #$blog_id.blog-posts .blog-post_title a {
				  ".$heading_font_size_css . $heading_line_height . $heading_font_weight ."
			  }";
	}
...
// Register css
if ( ! empty($styles) ) iGuru_shortcode_css()->enqueue_iguru_css($styles);
...
if ($blog_layout == 'carousel') {
	...
	if ((bool)$use_navigation) $carousel_options_arr['use_prev_next'] = 'true';
...
$row_class .= $hover_animation ? ' hover-animation' : '';
$row_class .= !empty($blog_content_align) ? ' a'.$blog_content_align : '';
...
if ( $query->have_posts() ) : ?>
	<section class="wgl_cpt_section">
		<div <?php if ((bool)$blog_id) echo 'id="'.esc_attr($blog_id).'"' ?> class="blog-posts <?php echo esc_attr($css_class); ?>">
			...
			if (!empty($blog_title) || !empty($blog_subtitle)) {
				...
				if (!empty($blog_title)) echo '<h3 class="iguru_module_title blog_title">'.wp_kses( $blog_title, $allowed_html ).'</h3>';
			...
			echo '<div class="container-grid row '. esc_attr($row_class) .'">';
				echo iGuru_Theme_Helper::render_html($blog_items);
			echo '</div>';
			?>
		</div>
		<?php
	...
	if ( $blog_navigation == 'pagination' ) {
		echo iGuru_Theme_Helper::pagination( '10', $query, $blog_navigation_align );
	}
	...
	if ( $blog_navigation == 'load_more' ) {
		...
		echo iGuru_Theme_Helper::load_more($wgl_blog_atts, $name_load_more, $class);
	}
	echo '</section>';
endif;
