...
$defaults = array(
	...
	// Content
	'descr_text' => '',
	'button_title' => esc_html__( 'Choose Plan', 'iguru' ),
	'link' => '',
	// Colors
	...
	'button_text_color_hover' => $theme_color_secondary,
	'button_bg_color' => $theme_color_secondary,
	'button_bg_color_hover' => '#ffffff',
	'button_bg_gradient_idle_start' => '#ffffff',
	'button_bg_gradient_idle_end' => '#ffffff',
	'button_bg_gradient_hover_start' => '',
	'button_bg_gradient_hover_end' => '',
	'button_border_color' => $theme_color_secondary,
	'button_border_color_hover' => $theme_color_secondary,
	'button_border_gradient_idle_start' => '',
	'button_border_gradient_idle_end' => '',
	'button_border_gradient_hover_start' => '',
	'button_border_gradient_hover_end' => '',
	// Typography
	'title_size' => '',
	'title_weight' => '',
	'price_size' => '',
...
$pricing_icon_out = $icon_output = $pricing_plan_id_attr = '';
...
if ((bool)$custom_icon_color || (bool)$custom_price_color || (bool)$custom_description_color || (bool)$custom_title_color || $pricing_customize != 'def' || !empty($header_customize) || !empty($content_customize) || !empty($footer_customize)) {
	$pricing_plan_id = uniqid( "iguru_pricing_plan_" );
...
ob_start();
	...
	if ((bool)$custom_price_color) {
		echo "#$pricing_plan_id .pricing_price_wrap .pricing_price {
	...
	if ((bool)$custom_description_color) {
		echo "#$pricing_plan_id .pricing_desc {
				  color: ".(!empty($description_bg_color) ? esc_attr($description_color) : 'transparent').";
			  }";
		echo "#$pricing_plan_id .pricing_desc {
...
// Wrapper classes
$pricing_wrap_classes = (bool)$shadow ? ' shadow' : '';
...
// Render Google Fonts
extract( iGuru_GoogleFontsRender::getAttributes( $atts, $this, array('google_fonts_title') ) );
...
$p_title_out = !empty($pricing_title) ? '<h4 class="pricing_title"'.$title_style.'>'.esc_html($pricing_title).'</h4>' : '';

// Currency output
$p_currency_out = !empty($pricing_cur) ? '<span class="pricing_cur">'.esc_html($pricing_cur).'</span>' : '';

// Price output
if (isset($pricing_price)) :
	if ( $separate_decimal = false ) {
		preg_match( "/(\d+)(\.| |,)(\d+|[a-zA-Z]+)$/", $pricing_price, $matches, PREG_OFFSET_CAPTURE );
		switch (isset($matches[0])) {
			case true:
				$p_price_out = '<div class="pricing_price">';
					$p_price_out .= esc_html($matches[1][0]);
					$p_price_out .= '<span class="price_decimal">.'.esc_html($matches[3][0]).'</span>';
				$p_price_out .= '</div>';
				break;
			default:
				$p_price_out = '<div class="pricing_price">'.esc_html($pricing_price).'</div>';
				break;
		}
	} else {
		$p_price_out = '<div class="pricing_price">'.esc_html($pricing_price).'</div>';
...
$pricing_desc_out = sprintf( '<div class="pricing_desc"%s><span>%s</span></div>',
	$description_styles,
	esc_html($pricing_desc)
);
...
$pricing_inner = '<div class="pricing_header" '.$pricing_header_style.'>';
	...
	$pricing_inner .= $p_title_out;
	$pricing_inner .= '<div class="pricing_price_wrap"'.$price_styles.'>';
		$pricing_inner .= $p_currency_out;
		$pricing_inner .= $p_price_out;
	$pricing_inner .= '</div>';
...
$output = '<div'.$pricing_plan_id_attr.' class="iguru_module_pricing_plan'.esc_attr($pricing_wrap_classes).'">';
