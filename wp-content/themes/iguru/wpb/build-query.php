...
*
*
* @class        iGuru_Loop_Settings
* @version      1.0
* @category     Class
* @author       WebGeniusLab
*/
if (!class_exists('iGuru_Loop_Settings')) {
    class iGuru_Loop_Settings{
        ...
        public static function buildQuery($query){
            $query_builder = new iGuru_Query_Builder( $query );
        ...
        public static function init($base_name, $array = array()) {
            ...
            add_filter( 'vc_autocomplete_'.$base_name.'_by_posts_callback', 'iGuru_Loop_Settings::by_posts_suggester', 10, 2 );

            add_filter( 'vc_autocomplete_'.$base_name.'_by_posts_render', 'iGuru_Loop_Settings::by_posts_suggester_render', 10, 2 );

            // Add Autocomplete categories
            add_filter( 'vc_autocomplete_'.$base_name.'_categories_callback', 'iGuru_Loop_Settings::categories_suggester', 10, 1 );

            add_filter( 'vc_autocomplete_'.$base_name.'_categories_render', 'iGuru_Loop_Settings::categories_suggester_render', 10, 1 );

            // Add Autocomplete tags
            add_filter( 'vc_autocomplete_'.$base_name.'_tags_callback', 'iGuru_Loop_Settings::tags_suggester', 10, 1 );

            add_filter( 'vc_autocomplete_'.$base_name.'_tags_render', 'iGuru_Loop_Settings::tags_suggester_render', 10, 1 );            

            // Add Autocomplete taxonomies
            add_filter( 'vc_autocomplete_'.$base_name.'_taxonomies_callback', 'iGuru_Loop_Settings::taxonomies_suggester', 10, 2 );

            add_filter( 'vc_autocomplete_'.$base_name.'_taxonomies_render', 'iGuru_Loop_Settings::taxonomies_suggester_render', 10, 1 );          

            // Add Autocomplete users
            add_filter( 'vc_autocomplete_'.$base_name.'_author_callback', 'iGuru_Loop_Settings::author_suggester', 10, 1 );

            add_filter( 'vc_autocomplete_'.$base_name.'_author_render', 'iGuru_Loop_Settings::author_suggester_render', 10, 1 );
            ...
            vc_add_param(
                ...
                array(
                    ...
                    'heading'     => esc_html__( 'Post count', 'iguru' ),
                    'param_name'  => 'number_of_posts',
                    'std'         => 12,
                    'description' => '',
                    'group' => esc_html__( 'Query', 'iguru' ),
                )
            );
            vc_add_param(
                $base_name,
                array(
                    ...
                    'heading'     => esc_html__( 'Order By', 'iguru' ),
                    'param_name'  => 'order_by',
                    'value'       => array(
                        esc_html__( 'Date', 'iguru' )          => 'date',
                        esc_html__( 'Title', 'iguru' )         => 'title',
                        esc_html__( 'Author', 'iguru' )        => 'author',
                        esc_html__( 'Modified', 'iguru' )      => 'modified',
                        esc_html__( 'Random', 'iguru' )        => 'rand',
                        esc_html__( 'Comment count', 'iguru' ) => 'comment_count',
                        esc_html__( 'Menu order', 'iguru' )    => 'menu_order',
                    ),
                    'save_always' => true,
                    'description' => '',
                    'group' => esc_html__( 'Query', 'iguru' ),
            ...
            vc_add_param(
                ...
                array(
                    'type'        => 'dropdown',
                    'edit_field_class' => 'vc_col-sm-4 no-top-padding',
                    'heading'     => esc_html__( 'Sort Order', 'iguru' ),
                    'param_name'  => 'order',
                    'value'       => array(
                        esc_html__( 'ASC', 'iguru')  => 'ASC',
                        esc_html__( 'DESC', 'iguru') => 'DESC'
                    ),
                    'save_always' => true,
                    'description' => '',
                    'group' => esc_html__( 'Query', 'iguru' ),
            ...
            if(!isset($array['hide_cats'])){
                vc_add_param(
                    ...
                    array(
                        ...
                        'heading'     => esc_html__('Category Slug', 'iguru' ),
                        'param_name'  => 'categories',
                        'description' => esc_html__('Leave empty for all', 'iguru' ),
                        'group' => esc_html__( 'Query', 'iguru' ),
                        'settings' => array(
                            'multiple' => true,
                            'min_length' => 2,
                ...
                vc_add_param(
                    ...
                    array(
                        'type' => 'wgl_checkbox',
                        'heading' => esc_html__( 'Exclude', 'iguru' ),
            ...
            if(!isset($array['hide_tags'])){
                vc_add_param(
                    ...
                    array(
                        'type'        => 'autocomplete',
                        'edit_field_class' => 'vc_col-sm-10',
                        'heading'     => esc_html__('Tags Slug', 'iguru' ),
                        'param_name'  => 'tags',
                        'description' => esc_html__('Leave empty for all', 'iguru' ),
                        'group' => esc_html__( 'Query', 'iguru' ),
                        'settings' => array(
            ...
            vc_add_param(
                ...
                array(        
                    'type'        => 'autocomplete',
                    'edit_field_class' => 'vc_col-sm-10',
                    'heading'     => esc_html__( 'Taxonomies', 'iguru' ),
                    'param_name'  => 'taxonomies',
                    'description' => esc_html__( 'Filter output by custom taxonomies categories, enter category names here.', 'iguru' ),
                    'group' => esc_html__( 'Query', 'iguru' ),
                    'settings' => array(
                        'multiple' => true,
                        'min_length' => 2,
            ...
            if(!isset($array['hide_individual_posts'])){
                vc_add_param(
                    ...
                    array(           
                        'type'        => 'autocomplete',
                        'class'       => '',
                        'heading'     => esc_html__( 'Individual Posts/Pages/Custom Post Types', 'iguru' ),
                        'param_name'  => 'by_posts',
                        'description' => esc_html__( 'Individual Posts/Pages/Custom Post Types', 'iguru' ),
                        'settings' => array(
                            'multiple' => true,
                            'min_length' => 2,
                            'display_inline' => false,
            ...
            if(!isset($array['hide_author_posts'])){
                vc_add_param(
                    ...
                    array(          
                        'type'        => 'autocomplete',
                        'class'       => '',
                        'heading'     => esc_html__( 'Author', 'iguru' ),
                        'param_name'  => 'author',
                        'description' => esc_html__( 'Filter by author name.', 'iguru' ),
                        'group' => esc_html__( 'Query', 'iguru' ),
                        'edit_field_class' => 'vc_col-sm-10',
                        'settings' => array(
                            'multiple' => true,
         ...
         * @return array
         */
        public static function by_posts_suggester( $query, $base_name ) {
            $content = [];
            switch ($base_name) {
                case 'wgl_lp_courses': $post_type = 'lp_course'; break;
                default: $post_type = 'any'; break;
            }

            $args = ! empty($query) ? [ 's' => $query, 'post_type' => $post_type ] : [ 'post_type' => $post_type ];
            $args['numberposts'] = -1;
        ...
        public static function by_posts_suggester_render( $query ) {
            ...
            if ( ! empty( $query ) ) {
                $list = get_posts( ['post_type' => 'any', 'name' => $query] );
        ...
        public static function categories_suggester($query){
            ...
            foreach ( $categories as $cat ) {
                ...
                $content[] = array( 'value' => (string) $cat->slug, 'label' => $cat->cat_name.(!empty($parent) ? esc_html__( ' (Parent categories: (', 'iguru') .$parent.'))' : ""));
            }
            return $content;            
        }
         ...
         * @return array
         */
        public static function taxonomies_suggester( $query, $base_name ) {
            $content = [];
            ...
            switch ($base_name) {
                case 'wgl_lp_courses':
                    if ( in_array('course_category', $taxonomies) && in_array('course_tag', $taxonomies) ) 
                        $taxonomies = [ 'course_category', 'course_tag' ];
                    break;
            }

            $result = get_terms( $taxonomies, $search );
            ...
            foreach ( $result as $tag ) {
                ...
                $content[] = [
                    ...
                    'label' => $tag->name . ' (' . $tag->taxonomy . ')'.(!empty($parent) ? esc_html__( ' (Parent categories: (', 'iguru') .$parent.'))' : "")
                ];
            }
            return $content;
        }
            ...
            if ( ! empty( $query ) ) {
                ...
                foreach ( $list as $obj ) {  
                    ...
                    $content[] = array( 
                        ...
                        'label' => $obj->name . ' (' . $obj->taxonomy . ')'.(!empty($parent) ? esc_html__( ' (Parent categories: (', 'iguru') .$parent.'))' : "")
                    );
                }
                return isset($content[0]) ? $content[0] : false;  
            }
...
*
*
* @class        iGuru_Query_Builder
* @version      1.0
* @category Class
* @author       WebGeniusLab
*/
if (!class_exists('iGuru_Query_Builder')) {
    class iGuru_Query_Builder{
        /**
         * @since 1.0
         * @var array
        ...
        private function get_tax( $value ){
            ...
            $taxonomies = get_terms( iGuru_Loop_Settings::getTaxonomies() );
            ...
            $terms = get_terms( iGuru_Loop_Settings::getTaxonomies(),
