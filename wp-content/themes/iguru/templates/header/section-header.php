...
if (!class_exists('iGuru_get_header')) {
	class iGuru_get_header{
		...
		public function header_vars(){
			...
			$this->name_preset = iGuru_Theme_Helper::get_option('header_def_js_preset');
			$get_def_name = get_option( 'iguru_set_preset' );
			if ( !$this->in_array_r($this->name_preset, get_option( 'iguru_set_preset' ))) {
			...
			if (class_exists( 'RWMB_Loader' ) && $this->id !== 0) {
				...
				if ( ! empty($customize_header) && $customize_header != 'default') {
					if ( ! $this->in_array_r(rwmb_meta('mb_customize_header'), get_option( 'iguru_set_preset' ))){    	
					} else {
						$get_def_name = get_option( 'iguru_set_preset' );
		...
		*/
		public function header_class(){
			$header_shadow = iGuru_Theme_Helper::get_option( 'header_shadow',$this->name_preset, $this->def_preset );
			$header_on_bg = iGuru_Theme_Helper::get_option( 'header_on_bg',$this->name_preset, $this->def_preset );
		...
		*/	    
		...
		public function header_bar_editor($location = null,$position = null){
			...
			*/
			...
			${'header_'.$position.'_editor'} = iGuru_Theme_Helper::get_option($location.'_header_bar_'.$position.'_editor', $this->name_preset, $this->def_preset);
		...
		*/	    
		...
		public function header_bar_delimiter($k = null){
			...
			*/
			...
			$height = iGuru_Theme_Helper::get_option( 'bottom_header_delimiter'.$get_number.'_height', $this->name_preset, $this->def_preset );	
			$width = iGuru_Theme_Helper::get_option( 'bottom_header_delimiter'.$get_number.'_width', $this->name_preset, $this->def_preset );	    

			$bg_color = iGuru_Theme_Helper::get_option( 'bottom_header_delimiter'.$get_number.'_bg', $this->name_preset, $this->def_preset );

			$margin = iGuru_Theme_Helper::get_option( 'bottom_header_delimiter'.$get_number.'_margin', $this->name_preset, $this->def_preset );
			...
			if ( $this->html_render === 'sticky' ) {
				$custom_sticky = iGuru_Theme_Helper::get_option('bottom_header_delimiter'.$get_number.'_sticky_custom', $this->name_preset, $this->def_preset);
				if( !empty($custom_sticky) ){
					$bg_color = iGuru_Theme_Helper::get_option('bottom_header_delimiter'.$get_number.'_sticky_color', $this->name_preset, $this->def_preset);
				}
			}
		...
		*/	    
		...
		public function header_bar_button($k = null){
			...
			*/
			...
			$get_number = (int) filter_var($k, FILTER_SANITIZE_NUMBER_INT);
			$button_text = iGuru_Theme_Helper::get_option('bottom_header_button'.$get_number.'_title', $this->name_preset, $this->def_preset);	

			$link = iGuru_Theme_Helper::get_option('bottom_header_button'.$get_number.'_link', $this->name_preset, $this->def_preset);	
			
			$target = iGuru_Theme_Helper::get_option('bottom_header_button'.$get_number.'_target', $this->name_preset, $this->def_preset);	
			$target = !empty($target) ? 'target:%20_blank' : '';

			$size = iGuru_Theme_Helper::get_option('bottom_header_button'.$get_number.'_size', $this->name_preset, $this->def_preset);	
			
			$options_btn = $this->html_render === 'sticky' ? '_sticky' : '';

			$customize = iGuru_Theme_Helper::get_option('bottom_header_button'.$get_number.'_custom'.$options_btn, $this->name_preset, $this->def_preset);	

			$customize = empty($customize) ? 'def' : 'color';

			$bg_color = iGuru_Theme_Helper::get_option('bottom_header_button'.$get_number.'_bg'.$options_btn, $this->name_preset, $this->def_preset);	

			$bg_color = isset($bg_color['rgba']) ? $bg_color['rgba'] : '';

			$text_color = iGuru_Theme_Helper::get_option('bottom_header_button'.$get_number.'_color_txt'.$options_btn, $this->name_preset, $this->def_preset);	

			$text_color = isset($text_color['rgba']) ? $text_color['rgba'] : '';

			$border_color = iGuru_Theme_Helper::get_option('bottom_header_button'.$get_number.'_border'.$options_btn, $this->name_preset, $this->def_preset);	
			$border_color = isset($border_color['rgba']) ? $border_color['rgba'] : '';

			$bg_color_hover = iGuru_Theme_Helper::get_option('bottom_header_button'.$get_number.'_hover_bg'.$options_btn, $this->name_preset, $this->def_preset);	
			$bg_color_hover = isset($bg_color_hover['rgba']) ? $bg_color_hover['rgba'] : '';

			$text_color_hover = iGuru_Theme_Helper::get_option('bottom_header_button'.$get_number.'_hover_color_txt'.$options_btn, $this->name_preset, $this->def_preset);	
			$text_color_hover = isset($text_color_hover['rgba']) ? $text_color_hover['rgba'] : '';

			$border_color_hover = iGuru_Theme_Helper::get_option('bottom_header_button'.$get_number.'_hover_border'.$options_btn, $this->name_preset, $this->def_preset);	
		...
		*/	    
		...
		public function header_bar_spacer($location = null, $key = null){
			...
			*/
			...
			$spacer = iGuru_Theme_Helper::get_option($location.'_header_spacer'.$get_number, $this->name_preset, $this->def_preset);	    	
		...
		*/
		public function build_header_layout($section = 'bottom'){
			...
			$header_sticky_builder = iGuru_Theme_Helper::get_option('sticky_header');
			...
			$header_layout = iGuru_Theme_Helper::get_option($section.'_header_layout', $this->name_preset, $this->def_preset);
			$lavalamp_active = iGuru_Theme_Helper::get_option('lavalamp_active', $this->name_preset, $this->def_preset);
			...
			for ( $i = 1; $i <= $counter; $i++) {
				...
				if ( !empty($header_layout) && $has_element ) {
					echo '<div class="wgl-header-row wgl-header-row-section', esc_attr($location), '"', $this->row_style_color($location, $section), '>',
						'<div class="', esc_attr($this->row_width_class($location, $section)), '">',
							'<div class="wgl-header-row_wrapper"', $this->row_style_height($location, $section), '>';
		...
		private function row_width_class($s = '_middle', $section) {
			...
			*/
			...
			switch ($section) {
				case 'bottom':
					$width_container = iGuru_Theme_Helper::get_option('header'.$s.'_full_width',$this->name_preset, $this->def_preset);
				...
				case 'sticky':
					$width_container = iGuru_Theme_Helper::get_option('header_custom_sticky_full_width');
					if ($width_container == '1') {
						$class = "fullwidth-wrapper";
					} else {
						$class = 'wgl-container';
		...
		private function row_style_color($s = '_middle', $section) {
			...
			* @access private
			*/
			$header_background = iGuru_Theme_Helper::get_option('header'.$s.'_background', $this->name_preset, $this->def_preset);
			$header_background_image = iGuru_Theme_Helper::get_option('header'.$s.'_background_image', $this->name_preset, $this->def_preset);
			$header_background_image = isset($header_background_image['url']) ? $header_background_image['url'] : '';  

			$header_color = iGuru_Theme_Helper::get_option('header'.$s.'_color', $this->name_preset, $this->def_preset);
			$header_bottom_border = iGuru_Theme_Helper::get_option('header'.$s.'_bottom_border', $this->name_preset, $this->def_preset);
			$header_border_height = iGuru_Theme_Helper::get_option('header'.$s.'_border_height', $this->name_preset, $this->def_preset);
			$header_border_height = $header_border_height['height'];
			$header_bottom_border_color = iGuru_Theme_Helper::get_option('header'.$s.'_bottom_border_color', $this->name_preset, $this->def_preset);
		...
		private function row_style_height($s = '_middle', $section) {
			...
			*/
			...
			$header_sticky_height = iGuru_Theme_Helper::get_option('header_sticky_height');
			$header_mobile_height = iGuru_Theme_Helper::get_option('header_mobile_height');
						
			$header_height = iGuru_Theme_Helper::get_option('header'.$s.'_height', $this->name_preset, $this->def_preset);
		...
		private function column_class($s = '_middle', $section, $area) {
			...
			* @access private
			*/
			$dispay = iGuru_Theme_Helper::get_option('header_column'.$s.'_'.$area.'_display', $this->name_preset, $this->def_preset);
			$v_align = iGuru_Theme_Helper::get_option('header_column'.$s.'_'.$area.'_vert', $this->name_preset, $this->def_preset);
			$h_align = iGuru_Theme_Helper::get_option('header_column'.$s.'_'.$area.'_horz', $this->name_preset, $this->def_preset);
		...
		*/
		public function build_header_mobile_menu($preset = null, $def_preset = null){
			...
			$header_queris = iGuru_Theme_Helper::get_option('header_mobile_queris' ,$preset, $def_preset);
		...
		public function header_render_html() {
			...
			echo "<header class='wgl-theme-header".esc_attr($this->header_class())."'>";
				...
				// LearnPress sign in/out form
				if ( $this->enable_sign_in_form ) $this->sign_in_form();
		...
		*/
		public static function get_logo($location){
			...
			$header_logo_src = iGuru_Theme_Helper::get_option('header_logo');
			...
			// Get Sticky Logotype
			$logo_sticky_src =  iGuru_Theme_Helper::get_option('logo_sticky');
			...
			// Get Mobile Logotype
			$logo_mobile_src =  iGuru_Theme_Helper::get_option('logo_mobile');
			...
			$logo_height_custom = iGuru_Theme_Helper::get_option('logo_height_custom');
			$logo_height = iGuru_Theme_Helper::get_option('logo_height');
			$logo_height = $logo_height['height'];
			

			$sticky_logo_height_custom = iGuru_Theme_Helper::get_option('sticky_logo_height_custom');
			$sticky_logo_height = iGuru_Theme_Helper::get_option('sticky_logo_height');
			$sticky_logo_height = $sticky_logo_height['height'];

			$mobile_logo_height_custom = iGuru_Theme_Helper::get_option('mobile_logo_height_custom');
			$mobile_logo_height = iGuru_Theme_Helper::get_option('mobile_logo_height');
			...
			switch (true) {
				case $location == 'bottom':
					if (!empty($header_logo_src)) {
						?>
						<img class="default_logo" src="<?php echo esc_url($header_logo_src); ?>" alt="<?php echo esc_attr($def_img_alt); ?>" <?php echo iGuru_Theme_Helper::render_html($logo_height_style);?>>
					<?php
					} else {
						?>
						<h1 class="logo-name">
				...
				case !empty($logo_sticky_src) && $location == 'sticky':
					?>
					<img class="logo-sticky" src="<?php echo esc_url($logo_sticky_src);?>" alt="<?php echo esc_attr($sticky_img_alt); ?>" <?php echo iGuru_Theme_Helper::render_html($sticky_height_style);?>>
					<?php
					break;
				case !empty($logo_mobile_src) && $location == 'mobile':
					?>
					<img class="logo-mobile" src="<?php echo esc_url($logo_mobile_src);?>" alt="<?php echo esc_attr($mobile_img_alt); ?>" <?php echo iGuru_Theme_Helper::render_html($mobile_height_style);?>>
					<?php
					break;
				default:
					if (!empty($header_logo_src)) {
		...
		*/
		public function search($html_render = '',$location, $section){
			$description = esc_html__( 'Search', 'iguru' );
			$search_style = iGuru_Theme_Helper::get_option('search_style');
			...
			$search_class = ' search_'.iGuru_Theme_Helper::get_option('search_style');
					...
					$output .= '<div class="header_search-field">';
						if ($search_style === 'alt') {
							$output .= '<div class="header_search-wrap">';
								$output .= '<div class="iguru_module_double_headings aleft">';
									$output .= '<h3 class="header_search-heading_description heading_title">'.apply_filters('iguru_desc_search', $description) .'</h3>';
								$output .= '</div>';
		...
		*/
		...
		/**
		* Get LearnPress Sign In/Out button
		*
		*
		* @since 1.0
		* @access protected
		*/
		protected function sign_in_button($location, $section) {
			...
			echo '<div class="log-in learnpress"', $this->row_style_height($location, $section), '>';
				if ( $user->is_guest() ) :
					$class = ' sign-in';
					...
					echo
						'<a class="log-link', $class,'" href="', esc_url_raw( $url ), '">',
							'<i class="flaticon-user-2"></i>',
							esc_html__( 'Sign In', 'iguru' ),
						'</a>'
					;
				else :
					$url = $profile->logout_url();
					echo
						'<a class="log-link sign-out" href="', esc_url_raw( $url ), '">',
							'<i class="flaticon-user-2"></i>',
							esc_html__( 'Sign Out', 'iguru' ),
						'</a>'
					;
				endif;
			echo '</div>';
		...
		* @access protected
		*/
		protected function sign_in_form() {
			$profile = LP_Global::profile();
			$is_guest = $profile->get_user()->is_guest();

			// Success/error messages
			if ( 'yes' === LP()->session->get( 'wgl_user_just_signed_in' ) ) {
				$user_name = wp_get_current_user()->display_name ? wp_get_current_user()->display_name : wp_get_current_user()->nickname;
				if ( $user_name ) {
					$message = sprintf( esc_html__( 'Hello, %s!', 'iguru' ), $user_name );
					learn_press_display_message( $message, 'success' );
				}
				LP()->session->remove( 'wgl_user_just_signed_in' );
			} else {
			...
			if ( $is_guest && $login_form = $profile->get_login_fields() ) :
				echo '<div class="wgl-sign_in_up_form">';
					echo
						'<ul class="sign-in-up-tabs">',
							'<li class="active">',
								'<a href="?tab=tab-login" data-tab=".tab-login">Login</a>',
							'</li>',
							'<li>',
								'<a href="?tab=tab-register" data-tab=".tab-register">Register</a>',
							'</li>',
						'</ul>'
					;
					...
					if ( $register_form = $profile->get_register_fields() ) :
		...
		*/
		...
		public static function icon_cart() {
			...
			?>
				<a class="woo_icon" title="<?php esc_attr_e( 'Open Shopping Cart', 'iguru' ); ?>">
