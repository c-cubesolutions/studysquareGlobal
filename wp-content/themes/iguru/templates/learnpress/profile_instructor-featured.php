...
$wrapper_class = $this_profile->main_class(false, 'iguru__instructor');
...
if ($is_instructor) :
	...
	$own_courses = $this_profile->query_courses()['items'];
	...
	$socials = [ 'twitter', 'facebook', 'linkedin', 'instagram', 'telegram' ];
	...
	foreach ($user_social as $soc_name => $soc_link) if ($soc_link) {
		$user_socials .= sprintf( '<a href="%s" class="social-link fa fa-%s"></a>', $soc_link, $soc_name );
	}
	! $user_socials || $user_socials = '<span class="social-link fa fa-share-alt"></span>'.$user_socials;
endif;

iGuru_LearnPress::logged_in_message();
...
printf( '<div %s>', $wrapper_class);
	...
	<div class="instructor__description"><?php
		...
		if ( $count_total_students || $count_total_courses || $my_total_reviews ) : ?>
		  <span class="instructor__dashboard"><?php
			if ( $count_total_students ) : ?>
			  <span class="dashboard--students">
				<i class="dashboard__icon flaticon-profile"></i>
				<span class="dashboard__data">
					<span class="data__title"><?php esc_html_e( 'Students', 'iguru' ); ?></span>
					<span class="data__value"><?php echo (int)$count_total_students; ?></span>
				</span>
			  </span><?php
			endif;
			if ( $count_total_courses ) : ?>
			  <span class="dashboard--courses">
				<i class="dashboard__icon flaticon-folder"></i>
				<span class="dashboard__data">
					<span class="data__title"><?php esc_html_e( 'Courses', 'iguru' ); ?></span>
					<span class="data__value"><?php echo (int)$count_total_courses; ?></span>
				</span>
			  </span><?php
			endif;
			if ( $my_total_reviews ) : ?>
			  <span class="dashboard--reviews">
				<i class="dashboard__icon flaticon-star"></i>
				<span class="dashboard__data">
					<span class="data__title"><?php esc_html_e( 'Reviews', 'iguru' ); ?></span>
