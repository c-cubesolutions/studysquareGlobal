...
*
*
* @class		iGuru_breadcrumbs
* @version		1.0
* @category		Class
* @author		WebGeniusLab
*/
if (!class_exists('iGuru_breadcrumbs')) {
    class iGuru_breadcrumbs {
		...
		private function text_options(){
			$this->text_home     = esc_html__( 'Home', 'iguru' ); // text for the 'Home' link
			$this->text_category = esc_html__( 'Archive by Category "%s"', 'iguru' ); // text for a category page
			$this->text_search   = esc_html__( 'Search Results for "%s" Query', 'iguru' ); // text for a search results page
			$this->text_tag      = esc_html__( 'Posts Tagged "%s"', 'iguru' ); // text for a tag page
			$this->text_author   = esc_html__( 'Articles Posted by %s', 'iguru' ); // text for an author page
			$this->text_404      = esc_html__( 'Error 404', 'iguru' ); // text for the 404 page
			$this->text_page     = esc_html__( 'Page %s', 'iguru' ); // text 'Page N'
			$this->text_cpage    = esc_html__( 'Comment Page %s', 'iguru' ); // text 'Comment Page N'
		}
		public function __construct () {
			...
			if (is_home() || is_front_page()) {
				...
				} elseif ( function_exists( 'learn_press_is_course_tag' ) && learn_press_is_course_tag() ) {
					if ($show_home_link) $out .= $sep;
					$tag_name = is_a( $obj = get_queried_object(), 'WP_Term') ? $obj->name : false;
					if ( ! $tag_name ) {
						global $wp_query;
						$tag = get_term_by( 'slug', $wp_query->query['course_tag'], 'course_tag' )  ;
						$tag_name = $tag->name;
					}
					if ( get_query_var('paged') ) {
						$out .= $sep . sprintf($link, get_tag_link($tag_id), $tag_name) . $sep . $before . sprintf($this->text_page, get_query_var('paged')) . $after;
					} else {
						if ($show_current) $out .= $before . sprintf( __( 'Courses tagged &ldquo;%s&rdquo;', 'iguru' ), $tag_name ) . $after;
					}
				...
				elseif (function_exists('is_product') && is_product()) {
	   				if (function_exists('iguru_woocommerce_breadcrumb')) {
	   					...
	   					$args['delimiter'] = $sep;
	   					if ($show_home_link) $out .= $sep;
	   					$out .= iguru_woocommerce_breadcrumb($args);
	   				}

				}
				$out .= $wrap_after;

				echo iGuru_Theme_Helper::render_html($out);
			}
    	}
    }
    new iGuru_breadcrumbs();
}