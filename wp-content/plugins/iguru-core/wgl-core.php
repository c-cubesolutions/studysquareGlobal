...
class iGuru_CorePlugin {
	...
	static function activation_check() {
		if ( ! self::compatible_version() ) {
			...
			wp_die( __( '"iGuru-core" Plugin is compatible with "iGuru" theme only!', 'iguru-core' ) );
		}
	}
	...
	function disabled_notice() {
	   echo '<strong>' . esc_html__( '"iGuru-core" Plugin compatible with "iGuru" theme only!', 'iguru-core' ) . '</strong>';
	} 
...
new iGuru_CorePlugin();

register_activation_hook( __FILE__, array( 'iGuru_CorePlugin', 'activation_check' ) );
 ...
 */
function deactivate_iguru_core() {
	require_once plugin_dir_path( __FILE__ ) . 'includes/class-wgl-core-deactivator.php';
	iGuru_Core_Deactivator::deactivate();
}

register_activation_hook( __FILE__, 'activate_iguru_core' );
register_deactivation_hook( __FILE__, 'deactivate_iguru_core' );
 ...
 * @since    1.0.0
 */
function run_iguru_core() {

	$plugin = new iGuru_Core();
	$plugin->run();
