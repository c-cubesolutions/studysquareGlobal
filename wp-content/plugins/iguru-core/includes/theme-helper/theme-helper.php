...
class WglThemeHelper{
	...
	public function render_post_share ($show_share) {
		...
		?>
			...
			<div class="share_social-wpapper">
				<a class="share_link share_twitter" target="_blank" href="<?php echo esc_url('https://twitter.com/intent/tweet?text='. get_the_title() .'&amp;url='. get_permalink()); ?>"><span class="fa fa-twitter" title="<?php echo esc_attr__( 'Share on Twitter', 'iguru-core' ); ?>"></span></a>
				<a class="share_link share_facebook" target="_blank" href="<?php echo  esc_url('https://www.facebook.com/share.php?u='. get_permalink()); ?>"><span class="fa fa-facebook" title="<?php echo esc_attr__( 'Share on Facebook', 'iguru-core' ); ?>"></span></a>
				<?php
					if (strlen($img_url[0]) > 0) {
						echo '<a class="share_link share_pinterest" target="_blank" href="'. esc_url('https://pinterest.com/pin/create/button/?url='. get_permalink() .'&media='. $img_url[0]) .'"><span class="fa fa-pinterest-p" title="'.esc_attr__( 'Share on Pinterest', 'iguru-core' ).'"></span></a>';
					}
				?>
				<a class="share_link share_linkedin" href="<?php echo esc_url('http://www.linkedin.com/shareArticle?mini=true&url='.substr(urlencode( get_permalink() ),0,1024));?>&title=<?php echo esc_attr(substr(urlencode(html_entity_decode(get_the_title())),0,200));?>" target="_blank" ><span class="fa fa-linkedin" title="<?php echo esc_attr__( 'Share on LinkedIn', 'iguru-core' ); ?>"></span></a>
			</div>
			<!-- // post share block -->
		<?php
		endif;
		...
		<div class="share_post-container">
			...
			<div class="share_social-wpapper">
				<ul>
					...
					if (strlen($img_url[0]) > 0) 
						printf( '<li><a class="share_post share_pinterest" href="%s" target="_blank"><span class="fa fa-pinterest"></span></a></li>',
							esc_url( 'https://pinterest.com/pin/create/button/?url='. get_permalink() .'&media='. $img_url[0] )
						);
					?>
					<li>
	...
	public function render_social_shares() {
		$description = esc_html__( 'Socials', 'iguru-core' );

		$facebook  = iGuru_Theme_Helper::options_compare('soc_icon_facebook', 'mb_customize_soc_shares', 'on');
		$twitter   = iGuru_Theme_Helper::options_compare('soc_icon_twitter', 'mb_customize_soc_shares', 'on');
		$linkedin  = iGuru_Theme_Helper::options_compare('soc_icon_linkedin', 'mb_customize_soc_shares', 'on');
		$pinterest = iGuru_Theme_Helper::options_compare('soc_icon_pinterest', 'mb_customize_soc_shares', 'on');
		$tumblr    = iGuru_Theme_Helper::options_compare('soc_icon_tumblr', 'mb_customize_soc_shares', 'on');

		$fixed  = iGuru_Theme_Helper::options_compare('soc_icon_position', 'mb_customize_soc_shares', 'on');
		$offset = iGuru_Theme_Helper::get_option('soc_icon_offset');
		...
		$share = iGuru_Theme_Helper::options_compare('soc_icon_style', 'mb_customize_soc_shares', 'on');
		...
		?>>
			<div class="share_social-wpapper"><?php
				if ($share == 'hovered'): ?>
					<div class="share_social-desc">
						<div class="share_social-title"><?php
							echo apply_filters('iguru_desc_socials', $description);
							?>
						</div>
