...
private $isMultilingual = FALSE; // Is this site multilingual?
	...
	function __construct() {
		parent::__construct(
			...
			esc_html__( 'WGL Banner', 'iguru-core' ), // Name
			[ 'description' => esc_html__( 'WGL Widget', 'iguru-core' ), ] // Args
		);
	...
	public function widget( $args, $instance ) {
		...
		if ('' === get_post_meta($attachment_id, '_wp_attachment_image_alt', true)) {
			$alt = the_title_attribute(array('before' => esc_html__('Featured image: ', 'iguru-core'), 'echo' => false));
		} else {
		...
		$widgetClasses = 'iguru_banner-widget iguru_widget widget';
		...
		$html = '<div class="' . esc_attr($widgetClasses) . '" style="'.esc_attr($widgetImg_2).'">';
			...
			if ( !empty($text_sub_title) )
				$html .= '<p class="banner-widget_text_sub">' . iGuru_Theme_Helper::render_html($text_sub_title) . '</p>';

			if ( !empty($text) ) {
				$html .= '<h2 class="banner-widget_text">' . iGuru_Theme_Helper::render_html($text);
				if ( !empty($show_tags_value) ) $html .= '<span class="tags_title"></span>';
				$html .= '</h2>';
			}

			if ( !empty($button_link) ) {
				$title = !empty($title) ? '<span>'. esc_html($title) . '</span>' : '';
				$html .= '<a class="banner-widget_button button-read-more" href="'.esc_url($button_link).'">'. $title.'</a>';
			}
	...
	public function form( $instance ) {
		...
		$defaults = [
			'show_tags'            => 'on',
			'padding_top_value'    => '56',
			'padding_bottom_value' => '73',
		];
		...
		<p>
		  <label for="<?php echo esc_attr( $this->get_field_id($image_name) ); ?>"><?php echo esc_html__('Add Logo image:', 'iguru-core')?></label><br />
		...
		<p>
		  <label for="<?php echo esc_attr( $this->get_field_id($image_name_2) ); ?>"><?php echo esc_html__('Add background image:', 'iguru-core')?></label><br />
			<img class="iguru_media_image" src="<?php if(!empty($instance[$image_name_2])){echo esc_url( $instance[$image_name_2] );} ?>" style="max-width: 100%" />
			<input type="text" class="widefat iguru_media_url" name="<?php echo esc_attr( $this->get_field_name($image_name_2) ); ?>" id="<?php echo esc_attr( $this->get_field_id($image_name_2) ); ?>" value="<?php echo esc_attr( $image_2 ); ?>">
			<a href="#" class="button iguru_media_upload"><?php esc_html_e('Upload', 'iguru-core'); ?></a>
		</p>

		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( $padding_top ) ); ?>"><?php echo esc_html__('Padding Top: ', 'iguru-core')?></label> 
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( $padding_bottom ) ); ?>"><?php echo esc_html__('Padding Bottom: ', 'iguru-core')?></label> 
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( $text_sub_title ) ); ?>"><?php echo esc_html__('Sub Title:', 'iguru-core')?></label> 
			<textarea class="widefat" id="<?php echo esc_attr( $this->get_field_id( $text_sub_title ) ); ?>" name="<?php echo esc_attr(  $this->get_field_name( $text_sub_title ) ); ?>" row="2"><?php echo iGuru_Theme_Helper::render_html($text_sub); ?></textarea>
		</p>

		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( $text_name ) ); ?>"><?php echo esc_html__('Title:', 'iguru-core')?></label> 
			<textarea class="widefat" id="<?php echo esc_attr( $this->get_field_id( $text_name ) ); ?>" name="<?php echo esc_attr(  $this->get_field_name( $text_name ) ); ?>" row="2"><?php echo iGuru_Theme_Helper::render_html($text); ?></textarea>
		</p>    
		...
		<p>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_tags')); ?>"><?php esc_html_e( 'Show Title Tags' , 'iguru-core' ); ?></label>
		</p>           

		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( $title_name ) ); ?>"><?php echo esc_html__('Button text:', 'iguru-core')?></label> 
		...
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'button_link' ) ); ?>"><?php esc_html_e( 'Button link:', 'iguru-core' ); ?></label> 
