...
private $isMultilingual = FALSE; // Is this site multilingual?
    ...
    function __construct() 
    {
        parent::__construct(
            ...
            esc_html__( 'WGL Author Said', 'iguru-core' ), // Name
            array( 'description' => esc_html__( 'WGL Widget ', 'iguru-core' ), ) // Args
        );
    ...
    public function enqueue_backend_scripts()
    {
        ...
        wp_enqueue_script('iguru-upload-img', get_template_directory_uri() . '/core/admin/js/img_upload.js', array('jquery'), false, true);
    }
    ...
    public function widget( $args, $instance ) 
    {
        ...
        if ('' === get_post_meta($attachment_id, '_wp_attachment_image_alt', true)) {
            $alt = the_title_attribute(array('before' => esc_html__( 'Featured author image: ', 'iguru-core' ), 'echo' => false));
        } else {
        ...
        if ('' === get_post_meta($attachment_id_s, '_wp_attachment_image_alt', true)) {
            $alt_s = the_title_attribute(array('before' => esc_html__( 'Featured author signature: ', 'iguru-core' ), 'echo' => false));
        } else {
            $alt_s = trim(strip_tags(get_post_meta($attachment_id_s, '_wp_attachment_image_alt', true)));
        }

        $widgetImgSign = $image_signature ? '<img class="author-widget_sign" src="' . esc_url(aq_resize($image_signature, '192', '95', true, true, true)) . '" alt="'.esc_attr($alt_s).'">' :'';
        ...
        $socials_arr = [ 'twitter', 'facebook', 'linkedin', 'instagram' ];
        foreach ( $socials_arr as $social) {
            ${$social} = ( (isset( $instance[$social])) && (!empty($instance[$social])) ) ? $instance[$social] : '';
        }
        
        $widgetClasses = 'iguru_author-widget';
        $widgetClasses .= ' widget iguru_widget';
        ...
        $html.= '<div class="author-widget_wrapper"'.(!empty($bg_image) ? ' style="'.esc_attr($bg_image).'"' : '').'>';
                if ( !empty($widgetImg) ) $html .= '<div class="author-widget_img-wrapper">' . $widgetImg . '</div>'; 
                if ( !empty($widgetImgSign) ) $html.= '<div class="author-widget_img_sign-wrapper">' . $widgetImgSign . '</div>';
                ...
                if ( !empty($facebook) || !empty($twitter) || !empty($linkedin) || !empty($instagram) ) {
                    ...
                    foreach ( $socials_arr as $social) {
                        if ( !empty(${$social}) ) $html .= '<a class="author-widget_social-link fa fa-'.$social.'" href="'.esc_url(${$social}).'"></a>';
                    }
        ...
        echo iGuru_Theme_Helper::render_html($html);
    }
    ...
    public function form( $instance ) {
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( $title_name ) ); ?>"><?php esc_html_e( 'Title:', 'iguru-core' ); ?></label> 
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( $author_name ) ); ?>"><?php esc_html_e( 'Author Name:', 'iguru-core' ); ?></label> 
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( $text_name ) ); ?>"><?php esc_html_e( 'Text:', 'iguru-core' ); ?></label> 
        ...
        <p>
          <label for="<?php echo esc_attr( $this->get_field_id($image_name) ); ?>"><?php esc_html_e( 'Author Image:', 'iguru-core' ); ?></label><br />
            <img class="iguru_media_image" src="<?php if(!empty($instance[$image_name])){echo esc_url( $instance[$image_name] );} ?>" style="max-width: 100%" />
            <input type="text" class="widefat iguru_media_url" name="<?php echo esc_attr( $this->get_field_name($image_name) ); ?>" id="<?php echo esc_attr( $this->get_field_id($image_name) ); ?>" value="<?php echo esc_attr( $image ); ?>">
            <a href="#" class="button iguru_media_upload"><?php esc_html_e( 'Upload', 'iguru-core' ); ?></a>
        </p>        

        <p>
          <label for="<?php echo esc_attr( $this->get_field_id($bg_image) ); ?>"><?php esc_html_e( 'Background Image:', 'iguru-core' ); ?></label><br />
            <img class="iguru_media_image" src="<?php if(!empty($instance[$bg_image])){echo esc_url( $instance[$bg_image] );} ?>" style="max-width: 100%" />
            <input type="text" class="widefat iguru_media_url" name="<?php echo esc_attr( $this->get_field_name($bg_image) ); ?>" id="<?php echo esc_attr( $this->get_field_id($bg_image) ); ?>" value="<?php echo esc_attr( $bg ); ?>">
            <a href="#" class="button iguru_media_upload"><?php esc_html_e( 'Upload', 'iguru-core' ); ?></a>
        </p>        

        <p>
          <label for="<?php echo esc_attr( $this->get_field_id($image_signature) ); ?>"><?php esc_html_e( 'Author Signature:', 'iguru-core' ); ?></label><br />
            <img class="iguru_media_image" src="<?php if(!empty($instance[$image_signature])){echo esc_url( $instance[$image_signature] );} ?>" style="max-width: 100%" />
            <input type="text" class="widefat iguru_media_url" name="<?php echo esc_attr( $this->get_field_name($image_signature) ); ?>" id="<?php echo esc_attr( $this->get_field_id($image_signature) ); ?>" value="<?php echo esc_attr( $signature ); ?>">
            <a href="#" class="button iguru_media_upload"><?php esc_html_e( 'Upload', 'iguru-core' ); ?></a>
        </p>

        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'facebook' ) ); ?>"><?php esc_html_e( 'Facebook:', 'iguru-core' ); ?></label> 
            <input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'facebook' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'facebook' ) ); ?>" type="text" value="<?php echo esc_attr( $facebook ); ?>">
        </p>
        
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'twitter' ) ); ?>"><?php esc_html_e( 'Twitter:', 'iguru-core' ); ?></label> 
            <input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'twitter' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'twitter' ) ); ?>" type="text" value="<?php echo esc_attr( $twitter ); ?>">
        </p>

        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'linkedin' ) ); ?>"><?php esc_html_e( 'Linkedin:', 'iguru-core' ); ?></label> 
            <input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'linkedin' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'linkedin' ) ); ?>" type="text" value="<?php echo esc_attr( $linkedin ); ?>">
        </p>        

        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'instagram' ) ); ?>"><?php esc_html_e( 'Instagram:', 'iguru-core' ); ?></label> 
