    ...
    class ReduxFramework_custom_preset{
        ...
        public function render() {
			...
			<div class="wgl_preset_notice">
				<?php
				esc_html_e( 'Notice:', 'iguru-core' );
				echo '<ul>';
					echo '<li>';
						echo '<div class="notice_h">';
						echo '</div>';
						esc_html_e( 'Adding of a new template will reset all unsaved changes inside Header Builder.', 'iguru-core' );
					echo '</li>';
					echo '<li>';
						esc_html_e( 'Select the Template to start setting it up.', 'iguru-core' );
					echo '</li>';
					echo '<li>';
						esc_html_e( 'Be sure to click "Save Preset" button after modifying of each template.', 'iguru-core' );
					echo '</li>';
				echo '<ul>';
				?>
			</div>
            <div id="wgl_custom_preset" class="wgl_custom_preset-wrapper">
            <div id="redux-add-preset-action"><input type="submit" id="redux-add-preset" value="<?php esc_html_e( 'Add New Template', 'iguru-core' ) ?>"><i class="fa fa-plus"></i></div>
            ...
            <div id="redux-save-preset-action" style="display:none;">
                <div class="wrap-save-preset">
                    ...
                    <input type="submit" id="redux-save-preset" class="button-primary" value="<?php esc_html_e( 'Save', 'iguru-core' ) ?>">                    
                </div>
            ...
            if ( isset( $this->field['options'] ) ) {
                if(!empty($this->field['options'])){
                    echo "<h4>".__('TEMPLATES LIST', 'iguru-core')."</h4>";
                }
                    ...
                    echo '<div class="wrapper-btn-preset">';                        
                        ...
                        if ( $selected ) { 
                            echo '<div id="selected-btn-preset" class="btn-preset">';                                               
                                echo '<input type="submit" class="selected-btn_custom btn-primary redux-update-preset" value="'.apply_filters( "wgl-changed-text-save-preset-{$this->parent->args['opt_name']}", __( 'Save Preset', 'iguru-core' ) ).'">'; 
                                
                            echo '</div>';
                        }   
                        
                        if(!$v['default'] && !array_key_exists($k, $list_defualt_preset)){ 
                            echo '<input type="submit" class="selected-btn_custom btn-primary redux-rename-preset" value="'.apply_filters( "wgl-changed-text-rename-preset-{$this->parent->args['opt_name']}", __( 'Rename', 'iguru-core' ) ).'">';

                             echo '<div class="delete-preset">';
                                echo '<a href="#" title="'.apply_filters( "wgl-changed-text-delete-preset-{$this->parent->args['opt_name']}", __( 'Delete Preset', 'iguru-core' ) ).'" class="redux-delete-preset">';
                        ...
                        if(!$v['default'] && array_key_exists($k, $list_defualt_preset)){
                        	if($selected){
	                            echo '<div class="delete-preset">';
	                                echo '<a href="#" title="'.apply_filters( "wgl-changed-text-reset-preset-{$this->parent->args['opt_name']}", __( 'Reset', 'iguru-core' ) ).'" class="redux-delete-preset reset-default">';
        ...
        public function enqueue() {
            ...
            $vars = array(
                'delete'   => __( 'Delete Preset?', 'iguru-core' ),
                'reset'   => __( 'Reset to default?', 'iguru-core' ),
                'taken_name'   => __( 'Sorry this name is already exists', 'iguru-core' ),
                'empty_name'   => __( 'Empty name', 'iguru-core' ),
            );
