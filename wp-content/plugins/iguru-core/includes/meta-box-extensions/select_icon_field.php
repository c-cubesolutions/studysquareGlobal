...
if ( class_exists( 'RWMB_Field' ) )
{
    ...
    class RWMB_Select_Icon_Field extends RWMB_Select_Field {
        ...
        public static function html( $meta, $field ) {
            ...
            $output = sprintf(
                '<div class="clone-select">'.esc_html__( 'Icon', 'iguru-core' ).'<br /><select %s>',
            ...
            $output .= '</select></div>';
            $meta_link = empty($meta['link']) ? '' : $meta['link'];
            $output .= '<label class="select-label">'. esc_html__( 'Link', 'iguru-core' ) .'<br />';
            $output .= '<input type="text" '.($field['clone'] ? 'class="rwmb-fieldset_text"' : '').' id="'.$field['id'].'" name="'.$attributes['name'].'[link]'.'" value="'.$meta_link.'">';
            $output .= '</label>';
        ...
        public static function normalize( $field ) {
            $field = wp_parse_args(
                ...
                [
                    'js_options'  => [],
                    'placeholder' => __( 'Select an icon', 'iguru-core' ),
                ]
            );
